...
if ($_GET['aksi'] == 'input') {
    ...
    $path = "images_bogor/" . $fotobaru;
    if ($judul != null and $keterangan != null and $foto != null) {
        if (move_uploaded_file($tmp, $path)) {
            $sql_input = "INSERT INTO `galeri_bogor` (`id`, `title`, `foto`, `keterangan`) VALUES "
                    . "(NULL, '$judul', '$fotobaru', '$keterangan')";
            $simpan_input = mysql_query($sql_input);
            if ($simpan_input) {
                header("location:index.php?page=galeri_bogor");
            } else {
    ...
    if ($foto != null) {
        ...
        if ($judul != null and $keterangan != null) {
            if (move_uploaded_file($tmp, $path)) {
                $sql_foto = "SELECT * from galeri_bogor WHERE id='$id'";
                $query_foto = mysql_query($sql_foto);
                $ambil_foto = mysql_fetch_array($query_foto);

                if (is_file("images_bogor/" . $ambil_foto['foto'])) {
                    unlink("images_bogor/" . $ambil_foto['foto']);
                }
                $sql_edit = "UPDATE galeri_bogor SET title = '$judul',keterangan='$keterangan',foto='$fotobaru' WHERE id = '$id' ";
        ...
        if ($judul != null and $keterangan != null) {
            $sql_edit = "UPDATE galeri_bogor SET title = '$judul',keterangan='$keterangan' WHERE id = '$id' ";
            $edit = mysql_query($sql_edit);
            if ($edit) {
                header("location:index.php?page=galeri_bogor");
            } else {
    ...
    $sql_hapus = "DELETE FROM galeri_bogor WHERE id = '$_GET[id]'";
