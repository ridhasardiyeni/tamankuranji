...
if ($_GET['aksi'] == 'input') {
    ...
    $path = "images_fasilitas/" . $fotobaru;
    if ($nama != null and $keterangan != null and $foto != null) {
        if (move_uploaded_file($tmp, $path)) {
            $sql_input = "INSERT INTO `fasilitas` (`id`, `nama`, `foto`, `keterangan`) VALUES "
                    . "(NULL, '$nama', '$fotobaru', '$keterangan')";
            $simpan_input = mysql_query($sql_input);
            if ($simpan_input) {
                header("location:index.php?page=fasilitas");
            } else {
    ...
    if ($foto != null) {
        ...
        if ($nama != null and $keterangan != null) {
            if (move_uploaded_file($tmp, $path)) {
                $sql_foto = "SELECT * from fasilitas WHERE id='$id'";
                $query_foto = mysql_query($sql_foto);
                $ambil_foto = mysql_fetch_array($query_foto);

                if (is_file("images_fasilitas/" . $ambil_foto['foto'])) {
                    unlink("images_fasilitas/" . $ambil_foto['foto']);
                }
                $sql_edit = "UPDATE fasilitas SET nama = '$nama',keterangan='$keterangan',foto='$fotobaru' WHERE id = '$id' ";
        ...
        if ($nama != null and $keterangan != null) {
            $sql_edit = "UPDATE fasilitas SET nama = '$nama',keterangan='$keterangan' WHERE id = '$id' ";
            $edit = mysql_query($sql_edit);
            if ($edit) {
                header("location:index.php?page=fasilitas");
            } else {
    ...
    $sql_hapus = "DELETE FROM fasilitas WHERE id = '$_GET[id]'";
