...
if ($_GET['aksi'] == 'input') {
    ...
    if ($judul != null and $keterangan != null and $url != null) {
        $sql_input = "INSERT INTO `video_bogor` (`id`, `title`, `url`, `keterangan`) VALUES "
                . "(NULL, '$judul', '$url', '$keterangan')";
        $simpan_input = mysql_query($sql_input);
        if ($simpan_input) {
            header("location:index.php?page=video_bogor");
        } else {
    ...
    $id = $_GET['id'];
    if ($judul != null and $keterangan != null and $url != null) {
        $sql_edit = "UPDATE video_bogor SET title = '$judul',keterangan='$keterangan', url = '$url' WHERE id = '$id' ";
    ...
    $sql_hapus = "DELETE FROM video_bogor WHERE id = '$_GET[id]'";
